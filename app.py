"""
============================================================
CODEX ALPHA — INSTITUTIONAL SCOUT PRO V33.1 (Minimal Check Slot)
...
def _conditional_entries(ws: dict, lv: dict, last: float, horizon: str):
    ...
    S = horizon == "short"
    stop_rng = round(lv["base_low"] * (0.985 if S else 0.98), 2)   # סטופ מבני לטווח
    cancel_all = f"סגירה יומית מתחת ${stop_rng} — המבנה נכשל, כל התוכנית מבוטלת"
...
def _render_war_room(ticker: str, result: dict, fd: dict) -> None:
    ...
    with col_fund:
        ...
        rows = [("תמחור", val or "—"),
                ...
                ("שולי רווח", f"{fd.get('operating_margin')}%" if fd.get("operating_margin") not in (None, "", "-") else "—"),
                ("איכות עסקית", grade)]
