"""
============================================================
CODEX ALPHA — INSTITUTIONAL SCOUT PRO V41.0 (Sets 4-5 · Horizon Parity)
Streamlit app for advanced Wyckoff-style market analysis
Optimized for Google Cloud Run

V41.0 — שני סטים בתוליים (4-5), 72 שמות, אפס חפיפה עם 100 הקודמים.
  המטרה: להביא את תוכניות הטווח הבינוני והארוך לאותה רמת ביטחון כמו הקצר.
  מיפוי הזיהום לפי החלטה (לא לפי סט): הורדת "נגיעה" הוכרעה מגילוי+אימות ·
  רצפות הסטופ 2.5%/4%/7% — שמגדירות את ההבדל בין הטווחים — הוכרעו מסט
  האימות · יציאת T1 מסט האימות. סט המבחן נקי לחלוטין (מדדנו, לא כיילנו).
  כלומר לקצר יש 3 מדידות ולבינוני/ארוך מדידה נקייה אחת בלבד.
  • סט 4: צריכה, תעשייה, הגנה, בנקאות אזורית וניהול-נכסים.
  • סט 5: מזון, כימיה, ציוד-שבבים, רשתות ותקשורת בינלאומית.
  • **הפרמטרים קפואים מרגע זה.** הסטים למדידה בלבד; טווח שיראה חולשה
    ידווח כחלש ולא יכויל. אחרת ניצור שני סטים שרופים ולא נשיג דבר.
...
# ── סטים 4-5 (V41.0) — נועדו להביא את הטווח הבינוני והארוך לאותה רמת
# ביטחון כמו הקצר. הקצר נמדד על 3 סטים; הבינוני והארוך על אחד בלבד
# (סט המבחן). שתי מדידות נקיות נוספות ⇒ 3 לכל טווח. הפרמטרים קפואים:
# הסטים האלה למדידה בלבד, אין כיול עליהם בשום מצב.
_BT_BLOCKS_SET4 = {
    1: ["CL", "KMB", "MO", "TJX"],
    2: ["ROST", "GWW", "FAST", "ITW"],
    3: ["ETN", "PH", "DOV", "CMI"],
...
def screen_backtest() -> None:
    ...
    _set = st.radio("סט נתונים", ["set4", "set5", "holdout", "validation", "discovery"],
                    horizontal=True,
    ...
    if _set in ("set4", "set5"):
        st.caption("🔒 סט בתולי — נועד להביא את תוכניות הטווח הבינוני והארוך לאותה "
                   "רמת ביטחון כמו הקצר. הקצר נמדד על 3 סטים בלתי-תלויים; הבינוני "
                   "והארוך על אחד בלבד. **הפרמטרים קפואים** — הסט הזה למדידה בלבד. "
                   "אם טווח יראה תוצאה חלשה, נדווח שהוא חלש ולא נכוון אותו. "
                   "הרץ פעם אחת.")
    elif _set == "holdout":
...
# V41.0 – סטים 4-5 בתוליים (72 שמות) להשוואת שלושת הטווחים ברמת ביטחון זהה. פרמטרים קפואים.
