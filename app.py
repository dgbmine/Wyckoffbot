"""
============================================================
INSTITUTIONAL SCOUT PRO V25.5 (Configurable Nasdaq Universe)
...
def inject_css() -> None:
    ...
    * { scrollbar-width: thin; scrollbar-color: var(--line-strong) var(--bg-0); }
    ...
    .orb-check .stButton > button, .orb-find .stButton > button, .orb-focus .stButton > button {
...
def classify_wyckoff_state(df: pd.DataFrame, tr: dict, events: list, vsa: list, weekly_ctx: dict) -> dict:
    ...
    if f["parabolic_broken"]:
        ...
        elif latest == "UTAD" or (f["has_bc"] and pos_val >= 0.6):
...
def _focused_filter(results, phase_state=None, grade_set=None, valuation_set=None,
                    ...
                    dura_fn=None, qadj_fn=None, progress_cb=None):
    """
    ...
    טהור וניתן לבדיקה (אפשר להזריק את כל הפונקציות). מחזיר (filtered, checked).
    """
    ...
    if mode == "landing":
        ...
        with col_find:
            ...
            st.markdown("</div><div class='home-orb-label'>תמצא לי</div>"
                        ...
                        unsafe_allow_html=True)
        # V25.4 — כפתור שלישי: סריקה ממוקדת (שילוב כל הצירים)
        st.markdown("<div style='height:14px;'></div>", unsafe_allow_html=True)
        _f1, col_focus, _f2 = st.columns([1, 2.4, 1])
        with col_focus:
        ...
        with b2:
            if st.button("↺ אפס הכל", use_container_width=True, key="focus_reset"):
                for k in ("focus_phase", "focus_sector", "focus_grades", "focus_vals",
                          "focus_raw", "focus_raw_sector", "focus_done"):
        ...
        if run_focus:
            try:
                ...
                if need_scan:
                    ...
                    def _scb(done, total, ticker, stats):
                        try:
                            _pb.progress(min(1.0, done / max(1, total)))
                            _ps.caption(f"סורק שוק {done}/{total} · עברו: {stats['passed']}")
                        except Exception:
                            pass

                    _mt = len(scan_universe) if sector_sel != ALL else 300
                    with st.spinner("סורק את השוק..."):
                        raw = scanner.scan_market(mode="balanced", max_tickers=_mt,
                                                  universe=scan_universe, top_n=60,
                                                  progress_callback=_scb)["results"]
                    _pb.empty()
                    _ps.empty()
                    st.session_state["focus_raw"] = raw
                    st.session_state["focus_raw_sector"] = sector_sel
                st.session_state["focus_done"] = True
            except Exception as exc:
                st.error(f"⚠️ שגיאה בסריקה: {exc}")
                st.session_state["focus_raw"] = []
                st.session_state["focus_raw_sector"] = sector_sel
                st.session_state["focus_done"] = True

        if st.session_state.get("focus_done"):
            # אם הסקטור הוחלף אחרי סריקה — צריך סריקה מחדש (היקום שונה)
            if st.session_state.get("focus_raw_sector") != sector_sel:
                st.info("שינית סקטור — לחץ '🎯 הרץ סריקה ממוקדת' לעדכון היקום. "
                        "(שינוי פאזה/איכות/תמחור מסנן מיידית ללא סריקה חוזרת.)")
            else:
                raw = st.session_state.get("focus_raw") or []
                if not raw:
                    st.info("לא נמצאו מניות שעברו את סינוני האיכות הבסיסיים של הסורק.")
                else:
                    _fpb = st.progress(0.0)
                    _fps = st.empty()

                    def _fcb(i, n, tk):
                        try:
                            _fpb.progress(min(1.0, i / max(1, n)))
                            _fps.caption(f"מסנן לפי הצירים {i}/{n} · {tk}")
                        except Exception:
                            pass

                    filtered, checked = _focused_filter(
                        raw, phase_state=phase_state, grade_set=grade_set,
                        valuation_set=valuation_set, sector_tickers=sector_tickers,
                        cap=40, progress_cb=_fcb)
                    _fpb.empty()
                    _fps.empty()
...
def _map_sectors_dict() -> dict:
    """V25.4 — מפת סקטורים→טיקרים ברמת המודול (מקור יחיד; משמש גם את הסריקה הממוקדת בבית)."""
    MAP_SECTORS = {
...
def _curated_fallback_universe() -> list:
    """היקום האצור המקורי (~146) — משמש כברירת מחדל וכמיזוג לשמירת סחורות/ETF חשובים."""
    universe = []
...
def _render_universe_status() -> None:
    ...
    meta = _universe_meta()
    if not meta:
        st.caption("🔭 יקום סריקה: רשימה אצורה (~146 מניות). להרחבה לכל נאסד\"ק המסונן — "
                   "הרץ build_universe.py והעלה את nasdaq_universe.json לצד app.py.")
        return
    n = meta.get("count", 0)
...
def screen_institutional_map() -> None:
    ...
    # === V25.3: אותם שני מסלולים גם בסריקה הסקטוריאלית — כללית / לפי פאזת Wyckoff ===
    sec_scan_type = st.radio(
        "סוג סריקה סקטוריאלית",
        options=["general", "phase"],
        format_func=lambda k: "🔍 סריקה כללית" if k == "general" else "🎯 סריקה לפי פאזת Wyckoff",
        key="sector_scan_type",
        horizontal=True,
    )
    sec_target_state = None
    if sec_scan_type == "phase":
        sec_target_state = st.selectbox(
            "בחר את הפאזה שאתה מחפש (חל על כל הסקטורים)",
            options=["ACC_BASE", "ACC_SPRING", "ACC_CONFIRM", "MARKUP",
                     "DIST_WARNING", "DIST_ACTIVE", "MARKDOWN"],
            format_func=lambda k: _WSTATES[k]["he"],
            key="sector_scan_phase",
        )
        st.caption("כל מועמד שעובר את סריקת הסקטור מאומת מול המנוע המבני (FSM) — יוצגו רק "
                   "מניות בפאזה שנבחרה. החלפת פאזה מסננת מיידית גם תוצאות סקטור קיימות.")
    ...
    for sector, data in MAP_SECTORS.items():
        with st.expander(f"{sector} ({len(data['tickers'])} מניות)", expanded=False):
                    ...
                    if not SCOUT_CORE_AVAILABLE:
                        ...
                        with st.spinner(f"סורק {len(data['tickers'])} מניות ב{sector} (Early Pruning)..."):
                            sec_out = scanner.scan_market(
                                ...
                                top_n=(12 if sec_scan_type == "phase" else 6),
                                progress_callback=_sec_cb,
                            )
                        prog_sec.progress(1.0)
                        st.session_state[scan_key] = sec_out["results"]
                    else:
                        with st.spinner(f"סורק {len(data['tickers'])} מניות ב{sector}..."):
                            st.session_state[scan_key] = scan_top_opportunities(
                                data["tickers"], top_n=(12 if sec_scan_type == "phase" else 6), mode="Balanced")
                except Exception as exc:
            ...
            if results is None:
                ...
                # V25.3: במצב פאזה — סינון מבני (FSM) של תוצאות הסקטור, מאותו מקור אמת
                if sec_scan_type == "phase" and sec_target_state:
                    _sp_label = _WSTATES[sec_target_state]["he"]
                    filtered, checked = _filter_results_by_phase(results, sec_target_state, cap=12)
                    st.caption(f"🎯 מסונן לפאזה: {_sp_label} — נמצאו {len(filtered)} מתוך {checked} מועמדי הסקטור.")
                    if not filtered:
                        st.info(f"אין כרגע מניות בפאזת '{_sp_label}' בסקטור זה. נסה סקטור/פאזה אחרים.")
                    results_view = filtered
                else:
                    results_view = results
                for r in results_view:
                    rcol1, rcol2 = st.columns([3, 1])
                    with rcol1:
                        # V25.3: תגית פאזה מבנית אחידה (מקור אמת יחיד) במקום pick_phase_caution הישן
                        r_cphase, r_timing, r_fit = _structural_timing_chip(r["ticker"], r.get("phase", ""), r.get("valuation", ""))
                        ...
                        st.markdown(
                            ...
                            f"תמחור: <b style='color:{r['valuation_color']}'>{r['valuation']}</b> · CIS {r['cis']:.0f} · ציון {r['composite']:.0f}<br>"
                            f"{r_timing} {r_fit}",
                            unsafe_allow_html=True,
                        )
                    with rcol2:
