"""
============================================================
INSTITUTIONAL SCOUT PRO V25.9 (Breakout Readiness — קרבה למהלך)
...
def _compute_days_in_phase(df: pd.DataFrame, tr: dict, is_bull: bool) -> int:
    """
    V25.9 — ימים ברצף בפאזה הנוכחית (ספירת רצף אחורה מהבר האחרון):
      • טווח (איסוף/הפצה): כמה ימים המחיר בתוך [תמיכה·0.97 .. התנגדות·1.03].
      • מגמה (Markup/Markdown): כמה ימים ברציפות מעל/מתחת ל-SMA20.
    proxy יציב למשך הפאזה מהנתונים הקיימים. עמיד-כשל (0 כברירת מחדל).
    """
    try:
...
def _breakout_readiness(ws: dict) -> dict:
    ...
    try:
        ...
        if state == "ACC_CONFIRM":
            ...
            lab, em, rk = "רחוק — בונה בסיס אחרי ניעור", "🟠", 2
            note = "שלב C (Spring) — הבסיס עדיין נבנה; דרך ארוכה עד פריצה."
        else:  # ACC_BASE
        ...
        if run_focus:
            try:
                ...
                if need_scan:
                    ...
                    _mt = min(len(scan_universe), 1500)  # כל היקום המובנה (515)
                ...
                with st.spinner("מסנן לפי הצירים שבחרת..."):
                ...
                st.session_state["focus_checked"] = checked
                ...
                for r in results_view:
                    ...
                    with rcol2:
                        if st.button("📊 ניתוח עומק", key=f"sector_deep::{sector}::{r['ticker']}", use_container_width=True):
                            st.session_state.home_mode = "check"  # V25.8 FIX: אחרת הבית נשאר במצב אחר וה-handoff לא נצרך
