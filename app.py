"""
============================================================
CODEX ALPHA — INSTITUTIONAL SCOUT PRO V32.1 (Context Gates + Replay)
...
def _render_horizon_strip(ticker: str, ws: dict, fd: dict) -> None:
    ...
    try:
        ...
        with _s2:
            ...
            if st.button("🧭 אסטרטגיית מסחר מלאה — 3 טווחים", key=f"strat_cta_{ticker}",
                         use_container_width=True):
                go_to_screen("🧭 אסטרטגיה", ticker)
...
def screen_trade_strategy() -> None:
    ...
    st.markdown("### 🧭 אסטרטגיית מסחר — שלוש תוכניות, שלושה טווחים")
...
def screen_home() -> None:
    ...
    if mode == "landing":
        ...
        # גישה לסורק הטכני (מפה מוסדית) — קישור עדין, לא סרגל ניווט
        _m1, _m2, _m3 = st.columns([1.5, 2, 1.5])
        with _m2:
            st.markdown("<div class='codex-mini' style='text-align:center; margin-top:6px;'>", unsafe_allow_html=True)
            if st.button("🗺️ הסורק הטכני — מפה מוסדית", key="codex_map_link", use_container_width=True):
                st.session_state.current_page = "🗺️ מפה מוסדית"
                st.rerun()
            st.markdown("</div>", unsafe_allow_html=True)
        return
...
def _render_screen_nav(screen: str, ticker: str = "") -> None:
    ...
    if screen == "fundamental":
        items += [("🔙 חזור לניתוח וואיקוף", "__wyckoff__"), ("🧭 אסטרטגיה", "🧭 אסטרטגיה")]
    elif screen == "scout":
        items += [("🔙 חזור לוואיקוף", "__wyckoff__"), ("🧭 אסטרטגיה", "🧭 אסטרטגיה")]
    elif screen == "strategy":
        items += [("🔙 לוואיקוף", "__wyckoff__"), ("📊 פונדמנטלי", "📊 ניתוח פונדמנטלי")]
...
def main() -> None:
    ...
    router = {
        "🏠 בית": screen_home,
        "🧭 אסטרטגיה": screen_trade_strategy,
