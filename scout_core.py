...
def synthesize_verdict(fund_data: dict, cis_score: float, current_phase: str, ticker: str = "") -> dict:
    ...
    if cis_score >= 55 and strong_cash:
        ...
        if start is not None and end is not None:
            ...
            vix_df = yf.Ticker("^VIX").history(start=start, end=end, auto_adjust=False)
            if vix_df is None or vix_df.empty: vix_df = yf.Ticker("^VIX").history(start=start, end=end)
        else:
            spy_df = yf.Ticker("SPY").history(period=period, auto_adjust=False)
            if spy_df is None or spy_df.empty: spy_df = yf.Ticker("SPY").history(period=period)
            vix_df = yf.Ticker("^VIX").history(period=period, auto_adjust=False)
            if vix_df is None or vix_df.empty: vix_df = yf.Ticker("^VIX").history(period=period)
...
def get_fundamental_data(ticker: str, cis_score: float = None, current_phase: str = "") -> dict:
    ...
    אוכף סינתזה קשיחה לחלוטין (חוק הברזל) ללא מסרים חיוביים בפאזה דובית.
    """
    try:
        tkr = yf.Ticker(ticker)
        info = tkr.info or {}
        ...
        if not fin.empty and "Total Revenue" in fin.index and len(fin.columns) > 1:
            ...
            if pd.notna(rev_curr) and pd.notna(rev_prev) and rev_prev != 0:
                rev_growth = ((rev_curr - rev_prev) / abs(rev_prev)) * 100
            if "Operating Income" in fin.index and pd.notna(rev_curr) and rev_curr != 0:
                op_inc = fin.loc["Operating Income"].iloc[0]
                if pd.notna(op_inc):
                    op_margin = (op_inc / rev_curr) * 100
        ...
        if not bs.empty and not fin.empty:
            ...
            if pd.notna(total_debt) and pd.notna(cash) and pd.notna(ebitda) and ebitda > 0:
                net_debt_ebitda = (total_debt - cash) / ebitda
                
        peg_ratio = info.get("pegRatio", (fwd_pe / rev_growth) if rev_growth > 0 else 999)

        # Benchmarks מבוססי סקטור
        benchmarks = {
        ...
        if cis_score is not None:
            ...
            if is_bearish_phase or cis_score <= 40:
                is_toxic = True
                if high_debt or not strong_cash:
                    synthesis = f"☠️ Toxic Value Trap (סכין נופלת): הפאזה הטכנית ב-{ticker} היא '{current_phase}' והכסף החכם נוטש באגרסיביות. במקביל, ישנה חולשה תזרימית או מינוף חריג מול סקטור ה-{sector}. התרחק מיד!"
                else:
                    synthesis = f"🚨 סכין נופלת: הנתונים היבשים של {ticker} אולי נראים סבירים, אך הפאזה היא '{current_phase}' והמוסדיים זורקים סחורה (Distribution). אל תתפוס תחתיות. זו מלכודת ערך."
                ...
                if strong_cash and valuation != "יקר" and not high_debt:
                    ...
                    synthesis = f"🚀 פרמיית איכות: המוסדיים מוכנים לשלם פרמיה יקרה על {ticker} בזכות הנהלה שמדפיסה מזומן ומכה את הסקטור. הטרנד נתמך בעוצמה על ידי כסף חכם."
                elif high_debt or not strong_cash:
                    synthesis = f"⚠️ ספקולציית מומנטום: יש איסוף מוסדי טכני, אך החולשה במאזן או בתזרים של {ticker} מצביעה על מהלך טכני ספקולטיבי. סיכון גבוה להחזקה ארוכה."
            else:
                if cis_score >= 60 and strong_cash:
                    synthesis = f"⚖️ איסוף שקט: {ticker} מדפיסה מזומן ונאספת בהדרגה מתחת לרדאר (Phase B/C). המתנה לפריצת מחיר קרובה."
                else:
                    synthesis = f"💤 כסף מת: חוסר קצה פונדמנטלי וטכני ביחס למתחרות בסקטור ה-{sector}."

        # --- הסברים ממוקדים (Popovers) ---
        explanations = {
            "fcf_yield": f"תשואת תזרים חופשי (FCF Yield) של {ticker} עומדת על {fcf_yield:.1f}%. במודל הערך של Ackman, זהו הכסף האמיתי שהעסק מייצר ביחס לשווי השוק. מעל 3% נחשב לחוסן אדיר.",
            "peg_ratio": f"יחס PEG של {peg_ratio:.2f}. מכפיל זה משקלל את התמחור העתידי מול קצב צמיחת ההכנסות של {ticker}. מתחת ל-1.5 מצביע שצמיחת החברה מתומחרת בחסר.",
            "op_margin": f"שולי הרווח של {ticker} הם {op_margin:.1f}%. לעומת הממוצע בסקטור ה-{sector} ({bench['om']}%), זה מצביע על {'חפיר תחרותי חזק וכוח תמחור (Pricing Power)' if op_margin > bench['om'] else 'הוצאות תפעול כבדות או חוסר יכולת להתחרות בצורה יעילה'}.",
            "net_debt": f"יחס חוב ל-EBITDA עומד על {net_debt_ebitda:.2f}x. מודל מוסדי שמרני דורש מינוף מתחת ל-3x. {'המינוף בטוח וסביר בסביבת הריבית הנוכחית.' if net_debt_ebitda < 3 else 'זהירות: רמת המינוף הזו מהווה דגל אדום בוהק המייצר סיכון להשמדת ערך בעת משבר!'}"
        }
...
def calculate_advanced_metrics(trades: list, initial_capital: float = 100000.0) -> dict:
    if not trades:
        return {
            "max_drawdown": 0.0, "total_profit": 0.0, "total_trades": 0,
            "winning_trades": 0, "losing_trades": 0, "annual_pnl": {}, "wyckoff_success_rate": 0.0
        }
    ...
    return {
        "max_drawdown": max_drawdown, "total_profit": total_profit, "total_trades": total_trades,
        "winning_trades": winning_trades, "losing_trades": losing_trades,
        "annual_pnl": annual_profit, "wyckoff_success_rate": wyckoff_success_rate
    }
...
def calculate_phase_followthrough(df: pd.DataFrame, horizon: int = 20, threshold_pct: float = 0.04) -> dict:
    ...
    for i in range(n - horizon):
        ...
        if curr_phase == prev_phase: continue
        ...
        future_closes = closes[i+1 : i+1+horizon]
        if is_bull:
            ret = (np.max(future_closes) - closes[i]) / closes[i]
        else:
            ret = (closes[i] - np.min(future_closes)) / closes[i]
...
class FactorEngine:
    ...
    def compute(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        f["f04_absorption"] = (vol_ratio / spread_ratio) * (1.0 - ((df["Close"] - df["Low"].rolling(20).min()) / (df["High"].rolling(20).max() - df["Low"].rolling(20).min() + 1e-5)).clip(0, 1))
        f["f36_wyckoff_score"] = self._compute_quick_wyckoff(df)
    ...
    def composite_cis(self, factors: pd.DataFrame, df: pd.DataFrame = None) -> pd.Series:
        base_weights = {
            "f04_absorption": 4, "f07_obv_velocity": 4, "f14_inst_intent": 6, 
            "f20_liquidity_sweep": 5, "f26_accept_reject": 3, "f35_struct_break": 3, 
            "f_effort_vs_result": 4, "f_stopping_volume": 4, "f_reaccumulation": 3, "f_rs_spy": 4
        }
        dynamic_weights = {f: pd.Series(base_weights.get(f, 0), index=factors.index) for f in factors.columns if f in base_weights}
        score = pd.Series(0.0, index=factors.index)
        ...
        for i in range(60, len(df)):
            ...
            if c > s20 > s50 > s200 and c >= h60 * 0.95: 
                ...
                if c > l60 + a * 2 and c < s50 and v < v_ma * 0.8: 
                    ...
                    phases.iloc[i] = "TRANSITION / UNCERTAIN STATE"
            elif c < l60 * 1.05 and v > v_ma * 2.5 and c > df['Low'].iloc[i] + (df['High'].iloc[i] - df['Low'].iloc[i]) * 0.5: 
                phases.iloc[i] = "Phase A (Selling Climax)"
...
def generate_roadmap(current_phase: str) -> dict:
    ...
    if "Phase A" in current_phase: roadmap.update({"previous_phase": "ירידות (Markdown)", "next_phase": "שלב B", "action_plan": "חפש סימני ספיגה."})
    elif "Phase B" in current_phase: roadmap.update({"previous_phase": "שלב A", "next_phase": "שלב C (ניעור)", "action_plan": "היערך לניעור מטה (Spring)."})
    ...
    elif "Phase E" in current_phase or "Markup" in current_phase: roadmap.update({"previous_phase": "שלב D", "next_phase": "פיזור עליון (Distribution)", "action_plan": "נהל עם סטופ דינמי."})
...
def detect_failure_risks(df: pd.DataFrame, factors: pd.DataFrame, current_phase: str, accum_prob: float, allowed: bool, ticker: str) -> list:
    warnings_list = []
    if "Distribution" in current_phase or "Markdown" in current_phase:
        warnings_list.append("🔴 סכנת מחיקה: כסף חכם מפזר סחורה. אל תחפש תחתיות.")
    elif "Spring" in current_phase and _extract_last(factors, 'f07_obv_velocity') < 0:
        warnings_list.append("🔴 ניעור שקרי: ה-OBV יורד בחדות. זה אינו ניעור מוסדי אמיתי אלא זריקת סחורה.")
    elif _extract_last(factors, 'f_effort_vs_result') > 2.5 and df['Close'].iloc[-1] < df['Open'].iloc[-1]:
        warnings_list.append("⚠️ מוכר עקשן מעל המחיר (Supply Overhang): קונים מתאמצים ללא תוצאה (Effort vs Result גרוע).")
    if not warnings_list:
        warnings_list.append("✅ שמיים נקיים (Clear Skies): התנהגות המחיר וזרימת ההון תקינה לחלוטין.")
    return warnings_list

def generate_replay_analogies(ticker: str, current_phase: str, accum_prob: float, factors: pd.DataFrame) -> list:
    if "Phase C" in current_phase: return [f"🔍 הניעור הנוכחי ב-{ticker} מזכיר את ה-Spring של BTC בינואר 2023 - קצירת נזילות מהירה."]
    elif "Phase D" in current_phase: return [f"🔍 בניית כוח: {ticker} מזכירה את ההתבססות של NVDA רגע לפני הפריצה הגדולה שלה."]
    elif "Distribution" in current_phase: return [f"🔍 פיזור נזילות: דפוס הפיזור משכפל את התנהגות TSLA בסוף 2022."]
    return [f"🔍 שחיקה ואיסוף: מצב שקט המזכיר את התנהגות קרנות הגידור ב-AMZN באמצע 2023."]
